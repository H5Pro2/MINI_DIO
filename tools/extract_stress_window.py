...
def _window_metrics(rows: list[dict], start: int, size: int) -> dict[str, float]:
    ...
    return_volatility = (
        sum((value - (sum(signed_returns) / max(1, len(signed_returns)))) ** 2 for value in signed_returns)
        / max(1, len(signed_returns))
    ) ** 0.5
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extrahiere ein rohweltlich stark belastetes Kontrollfenster.")
    ...
    best = max(metrics, key=lambda item: item["stress_score"])
    ...
    payload = {
        ...
        "best": best,
        "top_windows": sorted(metrics, key=lambda item: item["stress_score"], reverse=True)[:10],
    }
