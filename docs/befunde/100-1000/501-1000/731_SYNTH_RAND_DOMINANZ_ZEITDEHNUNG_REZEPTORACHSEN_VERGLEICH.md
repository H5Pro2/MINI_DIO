# Synthetische Randdominanz-Zeitdehnung Rezeptorachsen-Vergleich

Passive Diagnose: echte Weltfolgen gegen Rezeptorachsen und hoechste Rohfeldaufnahme-Fenster.

## Weltmatrix

| Welt | Episoden | Zentrum | Offen | Rand/Kipp | Lautheit | Schaerfe | Rohfeld | Adaptfeld | Reduktion | Ratio | High-Offen | High-Rand/Kipp | High-Lautheit | High-Rohfeld | High-Reduktion |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| RAND_ZEIT_KOMPAKT | 3294 | 0.7860 | 0.1509 | 0.0070 | 0.1006 | 0.7820 | 0.0614 | 0.0533 | 0.0080 | 0.8690 | 0.7879 | 0.0697 | 0.4012 | 0.2244 | 0.0395 |
| RAND_ZEIT_GEDEHNT | 13194 | 0.8729 | 0.0894 | 0.0039 | 0.0729 | 0.7836 | 0.0480 | 0.0418 | 0.0063 | 0.8696 | 0.6909 | 0.0386 | 0.3458 | 0.1960 | 0.0345 |

## Befund

- Staerkste offene Variante im Hochlastfenster: `RAND_ZEIT_KOMPAKT` mit `0.7879`.
- Staerkste Rand/Kipp-Naehe im Hochlastfenster: `RAND_ZEIT_KOMPAKT` mit `0.0697`.
- Staerkste auditive Hochlast: `RAND_ZEIT_KOMPAKT` mit `0.4012`.
- Staerkste Adaptionsreduktion im Hochlastfenster: `RAND_ZEIT_KOMPAKT` mit `0.0395`.

## Ableitung

Die echte Weltpruefung trennt zwei Fragen: Wie klingt und wirkt die Welt insgesamt, und was passiert in den lautesten/rohesten Feldaufnahme-Fenstern?

Wenn Hochlastfenster offen oder randnaher werden, aber die adaptierte Feldaufnahme unter der Rohaufnahme bleibt, spricht das fuer dieselbe organische Aufnahmegrenze wie in den synthetischen Welten.
